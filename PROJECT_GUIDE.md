# 🐧 Lynvest — Master Project & Architecture Guide
> **Purpose of this document:** Complete, step-by-step documentation of Lynvest's architecture, release model, version sequencing, in-app update pipeline, and recent history. Designed for both human developers and AI assistants to pick up context immediately.

---

## 1. Project Overview

* **Application Name:** Lynvest
* **Repository:** [`darshancosmic/lynvest`](https://github.com/darshancosmic/lynvest)
* **Website:** [`lynvest.vercel.app`](https://lynvest.vercel.app) ([`darshancosmic/lynvest-website`](https://github.com/darshancosmic/lynvest-website))
...
Lynvest does **not** use fragmented package formats (`.deb`, `.rpm`, `.pkg.tar.zst`, or Flatpak). It utilizes a streamlined, universal Linux terminal distribution model that installs cleanly into user space without requiring root or `sudo` privileges.
...
* **Executable:** `~/.local/bin/lynvest`
* **Desktop Launcher:** `~/.local/share/applications/lynvest.desktop`
* **Application Icons:** `~/.local/share/icons/hicolor/<size>x<size>/apps/lynvest.png`
* **Database / Data Storage:** `~/.local/share/lynvest/` (or OS standard app data dir)
...
Lynvest features a fully automated, native in-app update check and 1-click update pipeline.

```mermaid
graph TD
    A[Lynvest Launches] --> B[Background Check version.json on GitHub raw]
    ...
    G --> H[Extracts & Atomically replaces ~/.local/bin/lynvest]
    H --> I[Banner switches to 'Restart Lynvest']
...
```json
{
  ...
  "tarball_url": "https://raw.githubusercontent.com/darshancosmic/lynvest/main/dist-packages/lynvest-0.1.1-linux-x86_64.tar.gz"
}
```

### 3.2 Rust Backend Architecture (`src-tauri/src/commands.rs`)
1. **`check_app_update()`**:
   - Queries `https://raw.githubusercontent.com/darshancosmic/lynvest/main/version.json` using `curl` with a 4s connection timeout.
...
2. **`install_app_update(download_url)`**:
   - Creates a clean staging directory in `/tmp/lynvest_update_staging/`.
   - Downloads the release tarball using `curl -fsSL`.
   - Extracts using `tar -xzf`.
   - Automatically searches for the executable binary `lynvest` directly and in nested archive folders.
   - Writes to staging binary `~/.local/bin/lynvest.new`, applies executable permissions (`chmod 0o755`), and atomically renames over `~/.local/bin/lynvest`.
   - Cleans up `/tmp`.
...
* **Top Banner (`src/components/UpdateBanner.tsx`):** Renders dynamically when `updateInfo?.has_update` is true. Features instant `[Update Now]` progress state and green pulsing `[Restart Lynvest]` prompt upon completion.
...
4. **`version.json`**:
   ```json
   {
     ...
     "tarball_url": "https://raw.githubusercontent.com/darshancosmic/lynvest/main/dist-packages/lynvest-0.1.2-linux-x86_64.tar.gz"
   }
   ```

### Step 2: Build and Package
Run these two commands in `/home/darshancosmic/Projects/lynvest`:
```bash
npm run release
bash packaging/build_packages.sh
```
*This compiles the optimized release binary and bundles `dist-packages/lynvest-0.1.2-linux-x86_64.tar.gz` alongside updated `dist-packages/SHA256SUMS`.*
...
**That's it!** All existing Lynvest installations worldwide will automatically detect `v0.1.2` upon their next launch and offer the 1-click update.
...
| **Buy Me a Coffee Widget** | Embedded floating support widget on the official website ([`lynvest.vercel.app`](https://lynvest.vercel.app)). |
...
│   ├── lynvest-0.1.0-linux-x86_64.tar.gz
│   ├── lynvest-0.1.1-linux-x86_64.tar.gz
│   └── SHA256SUMS
├── packaging/
│   ├── build_packages.sh            # Packages binary into universal tarball
│   └── lynvest.desktop              # Linux desktop launcher specification
├── src/                             # React frontend
│   ├── components/
